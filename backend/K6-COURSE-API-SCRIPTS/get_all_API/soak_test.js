...
export let options = {
  ...
  //  'request_count': ['count>=1', 'count<=8'], 
    'status_code': ['count>=0', 'count<=300000'],
    ...
    'http_reqs': ['count>=1', 'count<=300000'],
  },
...
export function setup() {
    const urls = [
      'http://localhost:8884/?order_by=id&sort=DESC',
      'http://localhost:8884/?order_by=id&sort=ASC'
    ];
...
export default function (data) {
    ...
    const responses = http.batch(urls.map(url => ({ url: url })));
